...
# Additionally the hook removes unnecessary css and js files (starting with '_' in the asset folder) if clean: true in _conf.yml
...
Jekyll::Hooks.register :site, :post_write do |site|
    if site.config["purify"]  
        ...
        Jekyll.logger.warn %x[purifycss #{css} #{js} #{html} #{force_class} #{compress} --info --out #{css}]
    end
